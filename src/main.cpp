...
// ──────────────────────────────────────────────────────────────
// 模式切換：預設完整模式；若定義 AUV_POWER_MINI 則進入迷你模式（只 IN1/IN2 + BUZZ）
// 在 config.h 內加： #define AUV_POWER_MINI 1  → 啟用迷你模式
...
const int BUZZ_DEF_FREQ   = 2000; // 2 kHz
...
#endif // !AUV_POWER_MINI

// ──────────────────────────────────────────────────────────────
// 抽水馬達（H-bridge 2 線，5kHz PWM）：-100..100（兩種模式皆有）
...
static void stopPumpSoft() {
  ...
  // 把腳位改回 OUTPUT，這樣 digitalWrite 才有推挽效果
  pinMode(IN1, OUTPUT);
  pinMode(IN2, OUTPUT);
  digitalWrite(IN1, LOW);
  digitalWrite(IN2, LOW);

  delay(20);  // 給 H-bridge 一點時間冷靜

  // 重新掛回 LEDC（之後 apply_pump_percent 會再寫 duty）
...
void setup() {
  ...
  // 預設中位
  ledcWrite(SERVO_CH, 0); // 稍後 first write 會覆蓋
  // 霍爾
  ...
  while (WiFi.status() == WL_CONNECTED) WiFi.disconnect(true);
  {
    ...
    if (!node.init(NODE_NAME)) {
      ...
      Serial.println("ROS 初始化失败");
    }
    delay(200);
  }
#if AUV_POWER_MINI
  // ROS 訂閱（共同：抽水馬達）
  node.add_int32_subscription("/system/cmd/pump_percent",  &cb_pump_percent_cmd);
  node.add_int32_subscription("/system/ctrl/dc_percent",   &cb_dc_percent_alias);
#endif

#if !AUV_POWER_MINI
  // 完整模式才訂閱/發佈
  node.add_int32_subscription("/power/cmd/pump_percent",  &cb_pump_percent_cmd);
  node.add_int32_subscription("/power/ctrl/dc_percent",   &cb_dc_percent_alias);
